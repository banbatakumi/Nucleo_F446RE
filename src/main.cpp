...
#define KD 250.000 // 姿制御微分ゲイン
#define PD_LIMIT 50 // 姿勢制御の最大パワー
#define MOVING_AVERAGE_FILTER_SAMPLE_NUMBER 5 // モーターの移動平均の取得回数
...
PwmOut motor_1_1(PA_10);
PwmOut motor_1_2(PB_3);
PwmOut motor_2_1(PB_5);
PwmOut motor_2_2(PA_11);
PwmOut motor_3_1(PA_8);
PwmOut motor_3_2(PB_10);
PwmOut motor_4_1(PB_6);
PwmOut motor_4_2(PA_9);
...
int main() {
    ...
    // メイン関数内の変数定義
    bool line_set = 0;
    uint8_t line_true = 0, pre_line = 0;
    ...
    uint8_t ball_follow_depth = 10;
    int16_t move_speed = 40, line_move_speed = 60;

    float dt = 0;
    
    float line_unit_vector_x[8] = { 0, 0, 0.5, 1, 0, 0, -0.5, -1};
    float line_unit_vector_y[8] = { 0.5, 1, 0, 0, -0.5, -1, 0, 0};
    float line_result_vector_x = 0, line_result_vector_y = 0;
    float pre_line_result_vector_x = 0, pre_line_result_vector_y = 0;
    int16_t line_back_angle = 0;
    ...
    while (true) {
        ...
        if (voltage_stop_count > VOLTAGE_STOP_COUNT_NUMBER_OF_TIMES) { // 電圧低下による停止
            ...
            if (mode == 0) { // UI操作時
                ...
                if (line_check[0] == 1 || line_check[1] == 1 || line_check[2] == 1 || line_check[3] == 1 || line_check[4] == 1 || line_check[5] == 1 || line_check[6] == 1 || line_check[7] == 1) { // いずれかのラインセンサが反応している時
                    line_timer.start();
                    line_timer.reset();
                    if (line_true == 0) line_brake_timer.start();
                    for (uint8_t count = 0; count < 8; count++) if (line_check[count] == 1) pre_line = count; // 最後に反応したラインセンサの記憶

                    if ((line_check[0] == 1 || line_check[1] == 1) && ((line_true == 3 && line_check[4] == 1) || line_true == 0)) line_true = 1; // 前外ライン
                    if (line_check[5] == 1 && line_true == 1) line_true = 5; // 前内ライン
                    if ((line_check[2] == 1 || line_check[3] == 1) && ((line_true == 4 && line_check[6] == 1) || line_true == 0)) line_true = 2; // 前外ライン
                    if (line_check[7] == 1 && line_true == 2) line_true = 6; // 前内ライン
                    if ((line_check[4] == 1 || line_check[5] == 1) && ((line_true == 1 && line_check[0] == 1) || line_true == 0)) line_true = 3; // 前外ライン
                    if (line_check[1] == 1 && line_true == 3) line_true = 7; // 前内ライン
                    if ((line_check[6] == 1 || line_check[7] == 1) && ((line_true == 2 && line_check[2] == 1) || line_true == 0)) line_true = 4; // 前外ライン
                    if (line_check[3] == 1 && line_true == 4) line_true = 8; // 前内ライン  
                }

                if (line_timer.read() > 0.1 && (((line_true == 1 || line_true == 5) && pre_line != 4) || ((line_true == 2 || line_true == 6) && pre_line != 6) || ((line_true == 3 || line_true == 7) && pre_line != 0) || ((line_true == 4 || line_true == 8) && pre_line != 2))) { // ライン処理から通常処理に戻る
                    line_true = 0;
                    line_timer.stop();
                    line_timer.reset();
                ...
                if (line_true != 0) {
                    for (uint8_t count = 0; count < 8; count++) if (line_check[count] == 1) {
                        line_result_vector_x = 0;
                        line_result_vector_y = 0;
                    }
                    for (uint8_t count = 0; count < 8; count++) line_result_vector_x += line_check[count] * line_unit_vector_x[count];
                    for (uint8_t count = 0; count < 8; count++) line_result_vector_y += line_check[count] * line_unit_vector_y[count];
                    if (pre_line_result_vector_x != line_result_vector_x || pre_line_result_vector_y != line_result_vector_y) line_back_angle = atan2(line_result_vector_x, line_result_vector_y) / PI * 180.00000 + 180.5;
                ...
                if (line_true != 0 && line_brake_timer.read() <= 0.05) {
                    motor_move(0, 0, 1);
                } else if (line_true == 1 || line_true == 5) { // 前ライン
                    motor_move(line_back_angle <= -90 || line_back_angle >= 90 ? line_back_angle : line_back_angle - 180, line_move_speed, 0);
                } else if (line_true == 2 || line_true == 6) { // 右ライン
                    motor_move(line_back_angle <= 0 && line_back_angle >= -180 ? line_back_angle : line_back_angle - 180, line_move_speed, 0);
                } else if (line_true == 3 || line_true == 7) { // 後ライン
                    motor_move(line_back_angle >= -90 && line_back_angle <= 90 ? line_back_angle : line_back_angle - 180, line_move_speed, 0);
                } else if (line_true == 4 || line_true == 8) { // 左ライン
                    motor_move(line_back_angle >= 0 && line_back_angle <= 180 ? line_back_angle : line_back_angle - 180, line_move_speed, 0);
                } else {
                    if (abs(blue_angle) > 75) { // コートの端
                        ...
                        motor_move(abs(ball_angle) > BALL_FOLLOW_RANGE ? ball_angle + (ball_angle > 0 ? 90 : -90) : ball_angle * ((90 + BALL_FOLLOW_RANGE) / BALL_FOLLOW_RANGE) * (((ball_distance - ball_follow_depth < 1 ? 1 : ball_distance) - ball_follow_depth) / float(100.000 - ball_follow_depth)), move_speed, 0); // 回り込み
                    }
                }
            } else if (mode == 2) { // モード２
                motor_move(ball_angle > 0 ? (line_check[0] == 1 || line_check[1] == 1 || line_check[4] == 1 || line_check[5] == 1 ? 30 : 110) : (line_check[0] == 1 || line_check[1] == 1 || line_check[4] == 1 || line_check[5] == 1 ? -30 : -110), ball_angle * ball_angle / 15 > move_speed ? move_speed : ball_angle * ball_angle / 10, 0);
            } else if (mode == 3) { // モード３
            } else if (mode == 4) { // モード４
                motor_move(0, 0, 0);
            }
...
void ui(int8_t* select, int8_t display_mode, int8_t* set_mode, int8_t* set_value, int8_t* mode, float voltage_value, bool* line_set, int16_t ball_angle, int16_t ball_distance, int16_t* move_speed, int16_t* line_move_speed, uint16_t* line_threshpre, float dt, float* ball_rc, uint8_t* ball_follow_depth) { // UI
    ...
    if (display_mode == 0) {
        ...
        if (*select == 0) {
            ...
            if (line_check[0] == 1) oled.printf("1");
            else oled.printf("O");
            oled.setTextCursor(20, 10);
            if (line_check[1] == 1) oled.printf("1");
            else oled.printf("O");
            oled.setTextCursor(40, 20);
            if (line_check[2] == 1) oled.printf("1");
            else oled.printf("O");
            oled.setTextCursor(30, 20);
            if (line_check[3] == 1) oled.printf("1");
            else oled.printf("O");
            oled.setTextCursor(20, 40);
            if (line_check[4] == 1) oled.printf("1");
            else oled.printf("O");
            oled.setTextCursor(20, 30);
            if (line_check[5] == 1) oled.printf("1");
            else  oled.printf("O");
            oled.setTextCursor(0, 20);
            if (line_check[6] == 1) oled.printf("1");
            else oled.printf("O");
            oled.setTextCursor(10, 20);
            if (line_check[7] == 1) oled.printf("1");
            else oled.printf("O");
        ...
        if (*select == 0) {
            ...
            oled.printf("dis: %d\n", ball_distance);
            oled.printf("rcf: %.2f\n", *ball_rc);
            for (uint16_t count = 0; count < 360; count += 3) oled.drawPixel(95 + (15 * cos(count * PI / 180.00000)), 32 + (15 * sin(count * PI / 180.00000)), 1);
            for (uint16_t count = 0; count < 360; count += 2) oled.drawPixel(95 + (30 * cos(count * PI / 180.00000)), 32 + (30 * sin(count * PI / 180.00000)), 1);
            ...
            for (uint16_t count = 0; count < 360; count += 10) oled.drawPixel(95 + ((110 - ball_distance) / 2.5 * cos((ball_angle - 90) * PI / 180.00000)) + (3 * cos(count * PI / 180.00000)), 32 + ((110 - ball_distance) / 2.5 * sin((ball_angle - 90) * PI / 180.00000)) + (3 * sin(count * PI / 180.00000)), 1);
            *set_mode = 2;
...
void ir_read(int16_t* ball_angle, int16_t* ball_distance, float ball_rc) { // IRセンサ値の取得
    ...
    for (uint8_t count = 0; count < IR_NUM; count++) for (uint8_t i = IR_SAMPLE_NUMBER - 1; i > 0; i--) sample_ir_value[i][count] = sample_ir_value[i - 1][count];
    for (uint8_t count = 0; count < IR_NUM; count++) sample_ir_value[0][count] = ir_value[count];
    for (uint8_t count = 0; count < IR_NUM; count++) ir_value[count] = 0;
    for (uint8_t count = 0; count < IR_NUM; count++) for (uint8_t i = 0; i < IR_SAMPLE_NUMBER; i++) ir_value[count] += sample_ir_value[i][count];

    for (uint8_t count = 0; count < IR_NUM; count++) ir_value[count] = (IR_READ_NUMBER_OF_TIME * IR_SAMPLE_NUMBER - ir_value[count]) * (1000.000 / (IR_READ_NUMBER_OF_TIME * IR_SAMPLE_NUMBER));
    for (uint8_t count = 0; count < IR_NUM; count++) ir_value[count] = ir_value[count] * (1 - ball_rc) + pre_ir_value[count] * ball_rc;
    for (uint8_t count = 0; count < IR_NUM; count++) pre_ir_value[count] = ir_value[count];
...
void line_read(bool line_set, uint16_t line_threshpre) { // ラインセンサ値の取得
    ...
    for (uint8_t count = 0; count < 8; count++) line_value[count] = line_value[count] * (1 - LINE_RC) + pre_line_value[count] * LINE_RC;
    for (uint8_t count = 0; count < 8; count++) pre_line_value[count] = line_value[count];

    // 反応したのかのチェック
    for (uint8_t count = 0; count < 8; count++) line_check[count] = line_value[count] > line_reaction[count] + line_threshpre ? 1 : 0;
}
...
void motor_move(int16_t move_angle, int16_t move_speed, uint8_t brake) {
    if (brake == 1) { // ブレーキ
        ...
        static int16_t pre_motor[MOVING_AVERAGE_FILTER_SAMPLE_NUMBER][4];
        int16_t maximum_motor = 0;
        ...
        if (move_speed > 0) for (int count = 0; count < 4; count++) motor[count] *= move_speed / maximum_motor;
        ...
        for (uint8_t count = 0; count < 4; count++) motor[count] += count < 2 ? pd : -pd;

        for (uint8_t count = 0; count < 4; count++) motor[count] = motor[count] > MOTOR_LIMIT ? MOTOR_LIMIT : motor[count]; // モーターの上限値超えた場合の修正
        
        for (uint8_t count = 0; count < 4; count++) for (uint8_t count1 = MOVING_AVERAGE_FILTER_SAMPLE_NUMBER - 1; count1 > 0; count1--) pre_motor[count1][count] = pre_motor[count1 - 1][count]; //過去n回分の値を記録
        for (uint8_t count = 0; count < 4; count++) pre_motor[0][count] = motor[count]; // 最新の値の取得
        for (uint8_t count = 0; count < 4; count++) for (uint8_t count1 = 0; count1 < MOVING_AVERAGE_FILTER_SAMPLE_NUMBER; count1++) motor[count] += pre_motor[count1][count];
        for (uint8_t count = 0; count < 4; count++) motor[count] /= MOVING_AVERAGE_FILTER_SAMPLE_NUMBER;
